...
from core.logger_setup import setup_logging_if_enabled
...
from control import window_controller
from core import utils
from app.discord_control import DiscordControlServer
from integrations.telegram_control import TelegramControl
...
from app.runtime_control import RuntimeControlWindow
from game.stage_manager import StageManager, load_image
from vision.state_finder import get_state, get_starr_nova_got_it_button_center, is_starr_nova_info_screen
from game.time_management import TimeManagement
...
def spectro_main(data):
    ...
    class Main:
        ...
        def __init__(self):
            ...
            self.you_got_check_interval = 0.4
            self.last_you_got_check = 0.0
            self.you_got_threshold = 0.8
            self.you_got_template = None  # ленивая загрузка (нужен scale_factor)
            ...
            self.telegram_control = TelegramControl(
                ...
                state_reader=lambda: {
                    "status":     "paused" if getattr(self, "was_paused", False) else "running",
                    "brawler":    getattr(self.Play, "current_brawler", "?"),
                    "trophies":   str(getattr(self.Stage_manager.Trophy_observer, "current_trophies", "?")),
                    "game_state": str(self.state or "?"),
                    "uptime":     f"{int((time.time() - self.start_time) / 60)}m",
                }
            )
        ...
        def handle_you_got_screen(self, frame):
            now = time.time()
            if now - self.last_you_got_check < self.you_got_check_interval:
                return False
            self.last_you_got_check = now

            # ленивая загрузка шаблона — scale_factor готов только после 1-го кадра
            if self.you_got_template is None:
                sf = self.window_controller.scale_factor
                if not sf:
                    return False
                try:
                    self.you_got_template = load_image("images/states/you_got.png", sf)
                except Exception as e:
                    print(f"YOU GOT template not loaded: {e}")
                    return False

            match = utils.find_template_center(frame, self.you_got_template,
                                               threshold=self.you_got_threshold)
            if not match:
                return False

            print("YOU GOT screen detected; tapping to dismiss.")
            self.window_controller.keys_up(list("wasd"))
            h, w = frame.shape[:2]
            self.window_controller.click(w // 2, h // 2, delay=0.08)
            return True
        ...
        def manage_time_tasks(self, frame):
            if self.handle_you_got_screen(frame):
                return
        ...
        def publish_control_details(self):
            try:
                current = self.Stage_manager.brawlers_pick_data[0] if self.Stage_manager.brawlers_pick_data else {}
                brawler = current.get("brawler", "-")
                trophies = getattr(self.Stage_manager.Trophy_observer, "current_trophies", current.get("trophies", "-"))
                self.control_window.update_details(
                    brawler=brawler,
                    trophies=trophies,
                    game_state=self.state or "starting",
                )
            except Exception:
                pass
        ...
        def main(self): #this is for timer to stop after time
            ...
            while True:
                if self.control_window.is_stop_requested():
                    print("Stop requested from control panel; stopping bot.")
                    break
                ...
                if abs(s_time - time.time()) > 1:
                    ...
                    if elapsed > 0:
                        ...
                        self.control_window.update_ips(self.ips_ema)
                        self.publish_control_details()
...
def run_app():
    ...
    app = App(login, SelectBrawler, spectro_main, all_brawlers, Hub)
    app.start(spectro_version, get_latest_version)
...
def write_crash_log(error):
    ...
    print(f"Spectro crashed during startup. Crash log saved to: {crash_path.resolve()}")
