...
from core.utils import _config_bool, load_toml_as_dict
...
def load_webhook_settings() -> dict[str, Any]:
    ...
    webhook_config.setdefault("username", "Spectro")
...
def _title_and_description(event_type: str, details: dict[str, Any]) -> tuple[str, str]:
    ...
    if event_type == "completed":
        return "🏆 All Targets Complete", "Spectro finished every queued target."
    if event_type == "bot_is_stuck":
        reason = str(details.get("reason") or "Spectro could not recover automatically.")
        return "🚨 Bot Needs Attention", reason
    if event_type == "test":
        return "🧪 Webhook Test", "Discord webhook is connected correctly."
    return "📣 Spectro Update", str(details.get("message") or "Bot event received.")
...
def _image_to_file(screenshot: Any) -> tuple[discord.File | None, str | None]:
    ...
    buffer.seek(0)
    return discord.File(buffer, filename="spectro_screenshot.png"), "attachment://spectro_screenshot.png"
...
async def async_notify_user(
    ...
    send_kwargs = {
        "embed": embed,
        "username": str(settings.get("username") or "Spectro"),
...
async def async_send_test_notification() -> bool:
    return await async_notify_user(
        ...
        details={
            "state": "configured",
            "message": "This is a manual test from the Spectro Hub.",
        },
    )
