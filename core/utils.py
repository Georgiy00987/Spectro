...
def load_brawler_data():
    """Load the saved brawler queue (list of dicts) from latest_brawler_data.json."""
    if os.path.exists("cfg/latest_brawler_data.json"):
        try:
            with open("cfg/latest_brawler_data.json", "r") as f:
                return json.load(f)
        except Exception as e:
            print(f"Could not read latest_brawler_data.json: {e}")
    return []
...
def fetch_brawl_stars_player_by_tag(player_tag, timeout=15, local_brawlers=None, current_trophies=None):
    """Получить трофеи бойцов БЕЗ токена — парсингом brawltracker.com по тегу.
    Возвращает форму, совместимую с официальным API:
    {"tag", "name", "brawlers": [{"name", "trophies", "power"}]}."""
    import importlib
    try:
        sync_mod = importlib.import_module("spectro.integrations.syncbrawlers2api")
    except Exception:
        sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
        sync_mod = importlib.import_module("spectro.integrations.syncbrawlers2api")
    tag = sync_mod.clean_tag(str(player_tag or ""))
    if not tag or tag == "YOURTAG":
        raise ValueError("Не задан player_tag в cfg/brawl_stars_api.toml.")
    if current_trophies is None and local_brawlers:
        try:
            current_trophies = local_brawlers[0].get("trophies")
        except (AttributeError, IndexError, TypeError):
            current_trophies = None
    html = sync_mod.fetch_html(tag, timeout=timeout, current_trophies=current_trophies)
    parsed = sync_mod.parse_brawltracker_html(html, tag)
    return {
        "tag": parsed.get("tag", f"#{tag}"),
        "name": parsed.get("player", "Player"),
        "brawlers": parsed.get("brawlers", []),
    }
...
def refresh_brawl_stars_api_token_if_enabled(config, file_path="cfg/brawl_stars_api.toml", force=False):
    ...
    key_name_prefix = str(config.get("key_name_prefix", "Spectro Auto")).strip() or "Spectro Auto"
    ...
    description = str(config.get("key_description", "Auto-generated by Spectro for the current public IP."))
    create_payload = {
        "name": key_name,
...
def _torch_ocr_gpu_available():
    """Return True only when EasyOCR can actually use a native Torch GPU backend.

    DirectML is used by ONNX Runtime in Spectro, but EasyOCR does not use torch-directml.
    Passing gpu=True to EasyOCR without CUDA/MPS only prints a noisy warning and falls
    back to CPU anyway, so we disable it unless Torch reports a real CUDA/MPS device.
    """
    try:
        import torch
...
class DefaultEasyOCR:
    def __init__(self):
        ...
        use_gpu = _torch_ocr_gpu_available()
        # EasyOCR prints "Using CPU. Note: This module is much faster with a GPU"
        # when Torch CUDA/MPS is unavailable. Spectro can still use GPU for ONNX via
        # DirectML/CUDA, so this EasyOCR message is confusing. Keep OCR on CPU quietly.
        try:
            self.reader = easyocr.Reader(['en'], gpu=use_gpu, verbose=False)
        except TypeError:
            # Older EasyOCR builds may not support verbose=False. Suppress only the
            # noisy initialization output, then keep normal runtime errors visible.
...
def save_brawler_data(data):
    ...
    """
    with open("cfg/latest_brawler_data.json", 'w') as f:
...
def fetch_brawl_stars_player(api_token, player_tag, timeout=15, current_trophies=None):
    from game.brawltracker_api import fetch_player_brawlers
    cleaned_tag = str(player_tag).strip()
    if not cleaned_tag or cleaned_tag.upper() == "#YOURTAG":
        raise ValueError("Не указан player_tag в cfg/brawl_stars_api.toml")
    return fetch_player_brawlers(cleaned_tag, timeout=timeout, current_trophies=current_trophies)
    ...
    if api_base_url != "localhost":
        ...
        if latest_version:
            ...
            if version.parse(current_version) < version.parse(latest_version):
                print(f"Warning: (ignore if you're using early access) You are not using the latest public version of Spectro. \nCheck the discord for the latest download link.")
        else:
...
async def async_notify_user(message_type: str | None = None, screenshot: Image = None, details=None) -> bool:
    from integrations.discord_notifier import async_notify_user as send_discord_notification
