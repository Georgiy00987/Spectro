...
if platform.system() != "Windows" or "microsoft" in platform.uname()[3].lower():
    print("\n" + "!"*50)
    print("  ERROR: This version of Spectro is for WINDOWS ONLY.")
...
def bootstrap():
    if os.environ.get("SPECTRO_BOOTSTRAP") == "1": return
    try:
        import setuptools
    except ImportError:
        print("\nDetected missing core tools. Stabilizing environment...")
        os.environ["SPECTRO_BOOTSTRAP"] = "1"
...
def ask_user(prompt_text):
    if os.environ.get("SPECTRO_SETUP_AUTO", "").strip().lower() in ("1", "true", "yes"):
...
if "--spectro-install" in sys.argv:
    try:
        setup_spectro()
...
if any(cmd in sys.argv for cmd in ["install", "develop"]):
    print(
        ...
        "Run 'python setup.py --spectro-install' or setup.exe instead."
    )
    try: setup_spectro()
