...
class Movement:
    ...
    def __init__(self, window_controller):
        ...
        self.navigator = GridNavigator(
            ...
            inflate=float(bot_config.get("nav_inflate", self.wall_path_padding + 6)),
        )
        ...
        self.angle_debounce_threshold = float(bot_config.get("angle_debounce_threshold", 18.0))
...
class Play(Movement):
    ...
    def run_playstyle(self, player_data, enemy_data, walls, brawler):
        ...
        env = {
            ...
            "is_enemy_hittable": self.is_enemy_hittable,    "is_enemy_hittable": self.is_enemy_hittable,
            # --- analog movement helpers (added) ---
            "angle_from_direction": self.angle_from_direction,
            "angle_opposite": self.angle_opposite,
            "find_best_angle": self.find_best_angle,
            "get_strafe_angle": self.get_strafe_angle,
            "apply_combat_dodge": self.apply_combat_dodge,
    ...
    def roam_to_teammate_or_forward(self, player_data, teammate_data, walls):
        # Analog (joystick-angle) roam used when no enemy is visible.
        # Returns a FLOAT angle in degrees (0=right, 90=down, 180=left,
        # 270=up) so the bot actually drives the joystick instead of
        # standing with it centered.
        player_pos = self.get_player_pos(player_data)

        # Walk straight toward the nearest teammate, just like chasing an
        # enemy.
        if teammate_data:
            closest_teammate, _ = self.get_closest_teammate(player_data, teammate_data)
            if closest_teammate is not None:
                ...
                return self.find_best_angle(player_pos, teammate_angle, walls)

        # Nobody around: head straight up the map, sliding around walls.
        return self.find_best_angle(player_pos, 270.0, walls)

    def _imitation_roam_angle(self, player_position):
        # Imitation policy (v1): proposes a learned roam direction when
        # there are no enemies. Returns a float angle or None (fall back to
        # the built-in forward roam). Never forces the bot to stand still.
        if not getattr(self, 'imitation_enabled', True):
            return None
        try:
            from control.imitation_policy import get_policy
        except Exception:
            return None
        pol = get_policy()
        if pol is None:
            return None
        data = getattr(self, '_imitation_data', None) or {}
        try:
            teammates = [self.get_enemy_pos(t) for t in (data.get('teammate') or [])]
        except Exception:
            teammates = []
        walls = []
        for b in (data.get('wall') or []):
            if len(b) >= 4:
                walls.append([(b[0] + b[2]) * 0.5, (b[1] + b[3]) * 0.5])
        ...
        if not w:
            w, h = 960, 544
        try:
            out = pol.predict(player_position, [], teammates, walls, w, h)
        except Exception as exc:
            print('[imitation] predict failed:', exc)
            return None
        if not out or out['move_class'] == 0:
            return None
        if out['confidence'] < getattr(self, 'imitation_conf_threshold', 0.35):
            return None
        return out['angle']

    def no_enemy_movement(self, player_data, walls):
        player_position = self.get_player_pos(player_data)
        imit_angle = self._imitation_roam_angle(player_position)
        if imit_angle is not None:
            return self.find_best_angle(player_position, imit_angle, walls)
    ...
    def navigate_heading(self, player_pos, target_pos, walls):
        """A* маршрут к target_pos в обход стен. None -> откат на старую логику."""
        if not getattr(self, "use_astar_navigation", False):
            return None
        try:
            return self.navigator.next_heading(
                player_pos, target_pos, walls,
    ...
    def _debounce_angle(self, angle: float, threshold_deg: float = 10.0) -> float:
        ...
        if threshold_deg is None:
            threshold_deg = getattr(self, "angle_debounce_threshold", 10.0)
    ...
    def loop(self, brawler, data, current_time):
        ...
        if self.is_showdown:
            ...
            if not self.is_there_enemy(data['enemy']):
                # No enemy in sight: never stand still. Roam toward a teammate
                # or push up the map (analog joystick) instead of going AFK.
                movement = self.roam_to_teammate_or_forward(data['player'][0], data.get('teammate'), data['wall'])
            else:
                movement = self.get_movement(player_data=data['player'][0], enemy_data=data['enemy'], walls=data['wall'], brawler=brawler, teammate_data=data['teammate'])
    ...
    def roam_movement(self, player_data, teammate_data, walls):
        """Ветка дерева 'врага нет'.
        - тиммейт далеко -> подтянуться к нему по A*
        - никого нет     -> идти ВВЕРХ по A* (270°), обходя стены
        """
        player_pos = self.get_player_pos(player_data)
        ...
        if getattr(self, "roam_up_enabled", True):
            ...
            if heading is not None:
                return heading
            if not self.is_path_blocked(player_pos, 'W', walls):
                return 270.0  # 270° = вверх

        # Узел 6: тупик -> старая логика
        return self.no_enemy_movement(player_data, walls)

    def follow_teammates_at_start(self, player_pos, teammate_data, walls):
        """Первые match_start_follow_seconds секунд матча идём к ближайшему
        тиммейту (группируемся), даже если рядом враги.
        None -> окно прошло, тиммейтов нет, или уже рядом -> обычная логика."""
        window = getattr(self, "match_start_follow_seconds", 0.0)
        if window <= 0:
            return None
        started = getattr(self, "match_start_time", 0.0)
        if started <= 0 or (time.time() - started) > window:
            return None
    ...
    def follow_teammates_above_movement(self, player_pos, enemy_data, teammate_data, walls, attack_range):
        """Режим: всё время идём за ближайшим тиммейтом, который ВЫШЕ нас на экране.
        None -> режим выкл, нет тиммейтов выше, уже рядом, или уступаем бою."""
        if not getattr(self, "follow_teammates_above", False):
            return None
        if not teammate_data:
            return None

        # уступаем бою: если враг уже в дистанции атаки — не мешаем драться
        if getattr(self, "follow_above_yield_to_combat", True) and self.is_there_enemy(enemy_data):
            enemy_coords, enemy_distance = self.find_closest_enemy(enemy_data, player_pos, walls, "attack")
            if enemy_coords is not None and enemy_distance <= attack_range:
                return None

        margin = getattr(self, "follow_above_margin", 20)
        best_pos, best_dist = None, float("inf")
        for tm in teammate_data:
            tm_pos = self.get_enemy_pos(tm)
            if tm_pos[1] < player_pos[1] - margin:  # выше = меньше Y
                d = self.get_distance(tm_pos, player_pos)
                if d < best_dist:
                    best_dist, best_pos = d, tm_pos

        if best_pos is None or best_dist <= self.teammate_follow_min_distance:
            return None  # никого выше / уже рядом
        return self.navigate_heading(player_pos, best_pos, walls)

    def _estimate_entity_hp(self, box, team):
        """Best-effort HP fraction (0..1) from the entity's health bar. None if unknown."""
        if box is None:
            return None
        frame = getattr(self, "current_frame", None)
        if frame is None:
            return None
        try:
            from hp_estimator import estimate_hp
            wc = getattr(self, "window_controller", None)
            sf = getattr(wc, "scale_factor", 1.0) or 1.0
            return estimate_hp(frame, box, team, scale_factor=sf, cfg=getattr(self, "hp_estimator_cfg", None))
        except Exception as e:
            if visual_debug:
                print(f"[HP] estimate failed: {e}")
            return None

    def _match_enemy_box(self, enemy_data, enemy_coords):
        """Return the enemy box whose center is closest to enemy_coords."""
        if not enemy_data or enemy_coords is None:
            return None
        best = None
        best_d = float("inf")
        for e in enemy_data:
            pos = self.get_enemy_pos(e)
            d = (pos[0] - enemy_coords[0]) ** 2 + (pos[1] - enemy_coords[1]) ** 2
            if d < best_d:
                best_d = d
                best = e
        return best

    def _hp_combat_decision(self, player_data, enemy_data, enemy_coords):
        """Returns 'retreat', 'finish', or None (no override). Fail-safe on unknown HP."""
        if not getattr(self, "hp_logic_enabled", False):
            return None
        own_hp = self._estimate_entity_hp(player_data, "self")
        if own_hp is None or own_hp > self.low_hp_threshold:
            return None
        enemy_box = self._match_enemy_box(enemy_data, enemy_coords)
        enemy_hp = self._estimate_entity_hp(enemy_box, "enemy")
        if enemy_hp is not None and enemy_hp <= self.enemy_low_hp_threshold:
            if visual_debug:
                print(f"[HP] own={own_hp:.2f} enemy={enemy_hp:.2f} -> FINISH")
            return "finish"
        if visual_debug:
            ehp = "?" if enemy_hp is None else f"{enemy_hp:.2f}"
            print(f"[HP] own={own_hp:.2f} enemy={ehp} -> RETREAT")
        return "retreat"
    ...
    def get_movement(self, player_data, enemy_data, walls, brawler, teammate_data=None):
        ...
        safe_range, attack_range, super_range = self.get_brawler_range(brawler)
        player_pos = self.get_player_pos(player_data)
        # Узел 0: первые секунды матча -> группируемся с тиммейтами
        start_follow = self.follow_teammates_at_start(player_pos, teammate_data, walls)
        if start_follow is not None:
            return start_follow
        # Узел 0b: режим "идти за тиммейтами, которые ВЫШЕ нас"
        above_follow = self.follow_teammates_above_movement(
            player_pos, enemy_data, teammate_data, walls, attack_range)
        if above_follow is not None:
            return above_follow
        if debug: print("found player pos:", player_pos)
        if not self.is_there_enemy(enemy_data):
            return self.roam_movement(player_data, teammate_data, walls)
        enemy_coords, enemy_distance = self.find_closest_enemy(enemy_data, player_pos, walls, "attack")
        if enemy_coords is None:
            return self.roam_movement(player_data, teammate_data, walls)
        ...
        direction_x = enemy_coords[0] - player_pos[0]
        direction_y = enemy_coords[1] - player_pos[1]
        # --- HP-aware decision: low own HP -> retreat, unless enemy also low -> finish ---
        hp_mode = self._hp_combat_decision(player_data, enemy_data, enemy_coords)
        if hp_mode == "finish":
            approach = True
        elif hp_mode == "retreat":
            approach = False
        else:
            approach = enemy_distance > safe_range
        # --- A* навигация: маршрут к врагу в обход стен ---
        movement = None
        if approach:  # подходим к врагу
            movement = self.navigate_heading(player_pos, enemy_coords, walls)
        if movement is None:
            # старая реактивная логика (отступление / запасной вариант)
            if approach:
