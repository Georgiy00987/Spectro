...
EMULATOR_PORTS = {
    ...
    "BlueStacks": [5555, 5556, 5565, 5575, 5585, 5595, 5605, 5554],
    "Nox": [62001, 62025, 62026, 62027, 62028, 5555, 5554],
    "MEmu": [21503, 21513, 21523, 21533, 21543, 5555, 5554],
    "GameLoop": [5555, 5554, 5556, 5557],
}
...
LOCAL_ADB_EXE = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "bin", "adb.exe")
_ADB_ONLINE_CACHE = {}
...
def _infer_supported_emulator(configured_port):
    ...
    if configured_port in (16384, 16416, 16448, 7555):
        return "MuMu"
    if configured_port in (5565, 5575, 5585, 5595, 5605):
        return "BlueStacks"
    if configured_port in (62001, 62025, 62026, 62027, 62028):
        return "Nox"
    if configured_port in (21503, 21513, 21523, 21533, 21543):
        return "MEmu"
    return "LDPlayer"
...
class WindowController:
        ...
        try:
            ...
            if not selected_is_preferred:
                print(
                    ...
                    f"using the best online supported emulator ADB device instead."
                )
    ...
    def _resolve_emulator_launch_command(self, general_config):
        ...
        if self.selected_emulator == "BlueStacks":
            player_path = str(general_config.get("bluestacks_player_path", "")).strip()
            if not player_path:
                player_path = _find_existing_path(COMMON_BLUESTACKS_PLAYERS)
            return [player_path] if player_path else None

        if self.selected_emulator == "Nox":
            player_path = str(general_config.get("nox_player_path", "")).strip()
            if not player_path:
                player_path = _find_existing_path(COMMON_NOX_PLAYERS)
            return [player_path] if player_path else None

        if self.selected_emulator == "MEmu":
            player_path = str(general_config.get("memu_player_path", "")).strip()
            if not player_path:
                player_path = _find_existing_path(COMMON_MEMU_PLAYERS)
            return [player_path] if player_path else None

        if self.selected_emulator == "GameLoop":
            player_path = str(general_config.get("gameloop_player_path", "")).strip()
            if not player_path:
                player_path = _find_existing_path(COMMON_GAMELOOP_PLAYERS)
            return [player_path] if player_path else None
    ...
    def _emulator_command_for(self, action):
        ...
        if self.selected_emulator in ("BlueStacks", "Nox", "MEmu", "GameLoop"):
            if action != "launch":
                return None
            return self.emulator_launch_command
