...
from core.utils import extract_text_and_positions, count_hsv_pixels, load_toml_as_dict, find_template_center

debug = load_toml_as_dict("cfg/general_config.toml")['super_debug'] == "yes"
gray_pixels_treshold = load_toml_as_dict("./cfg/bot_config.toml")['idle_pixels_minimum']


# Pairs of brawlers whose portraits look alike enough that template matching
# can confuse them. For these we raise the matching threshold and require the
# winner to clearly beat the runner-up before clicking.
_CONFLICT_PAIRS = [
    frozenset({"bolt",   "eve"}),
    frozenset({"bolt",   "moe"}),
...
_CONFLICTED_BRAWLERS = {key for pair in _CONFLICT_PAIRS for key in pair}
...
class LobbyAutomation:
    ...
    def select_brawler(self, brawler):
        ...
        try:
            search_cfg = self.coords_cfg.get("brawler_search", {}) or {}
        except AttributeError:
            search_cfg = {}
        max_scrolls = int(search_cfg.get("max_scrolls", 50))
        scroll_x_cfg = float(search_cfg.get("scroll_x", 1794))
        scroll_from_y = float(search_cfg.get("scroll_from_y", 900))
        scroll_to_y = float(search_cfg.get("scroll_to_y", 600))
        scroll_duration = float(search_cfg.get("scroll_duration", 0.45))
        first_scroll_sleep = float(search_cfg.get("first_scroll_sleep", 0.15))
        scroll_sleep = float(search_cfg.get("scroll_sleep", 0.8))
        edge_margin_frac = float(search_cfg.get("edge_margin_frac", 0.12))
        edge_margin_frac = max(0.0, min(0.3, edge_margin_frac))
        stable_repeats_to_stop = int(search_cfg.get("stable_repeats_to_stop", 2))
        ...
        def ocr_results(screenshot_full):
            ...
            for raw_key in results.keys():
                norm_key = self.resolve_ocr_typos(self.normalize_ocr_name(raw_key))
                reworked[norm_key] = results[raw_key]
            return reworked

        def ocr_match(reworked, h_full, ignore_edges):
            top_limit = h_full * edge_margin_frac
            bot_limit = h_full * (1.0 - edge_margin_frac)
            found = []
            for detected_name, text_box in reworked.items():
                cy_full = text_box['center'][1] / ocr_scale
                if (not ignore_edges) and (cy_full < top_limit or cy_full > bot_limit):
                    continue
                if self.names_match(detected_name, target_key):
                    found.append((self.name_match_score(detected_name, target_key), detected_name, text_box))
            found.sort(key=lambda item: item[0], reverse=True)
            return found

        def ocr_click_coords(text_box, h_full):
            cx, cy = text_box['center']
            click_x = int(cx / ocr_scale)
            ...
            click_y = int((cy / ocr_scale) - (95 * hr))
            click_y = max(0, min(h_full - 1, click_y))
        ...
        if debug_enabled:
            print(f"Brawler icon available for template match: {icon_available}")

        for i in range(max_scrolls):
            screenshot_full = self.window_controller.screenshot()
            h_full = screenshot_full.shape[0]
            ...
            if icon_match is not None:
                ...
                commit_selection(click_x, click_y, "ICON", f"{score:.2f}")
                found_brawler = True
                break

            # OCR fallback runs EVERY frame (even when an icon file exists) so a
            # missed/rejected template match does not skip the brawler.
            if debug_enabled:
                print("extracting text on current screen...")
            reworked_results = ocr_results(screenshot_full)
            if debug_enabled:
                print("All detected text while looking for brawler name:", list(reworked_results.keys()))
                print()

            matches = ocr_match(reworked_results, h_full, ignore_edges=False)
            if matches:
                _, detected_name, text_box = matches[0]
                click_x, click_y = ocr_click_coords(text_box, h_full)
                commit_selection(click_x, click_y, "OCR", detected_name)
                found_brawler = True
                break
            ...
            if stable_count >= stable_repeats_to_stop:
                # Bottom reached: final scan WITHOUT the edge guard so the last
                # row (which cannot scroll into the center) is not missed.
                final_matches = ocr_match(reworked_results, h_full, ignore_edges=True)
                if final_matches:
                    _, detected_name, text_box = final_matches[0]
                    click_x, click_y = ocr_click_coords(text_box, h_full)
                    commit_selection(click_x, click_y, "OCR-end", detected_name)
                    found_brawler = True
                elif debug_enabled:
                    print("Reached end of brawler list without a match.")
                break

            scroll_x = int(scroll_x_cfg * wr)
        ...
        if not found_brawler:
            print(f"WARNING: Brawler '{brawler}' was not found after {max_scrolls} scroll attempts. ")
            if reworked_results:
    ...
    def ensure_lobby_after_selection(self, timeout=6.0):
        ...
        while time.time() < deadline:
            ...
            if state == "brawler_selection":
                ...
                # Don't tap during an actual match / matchmaking.
                print(
