...
def download_python_installer():
    destination = Path(tempfile.gettempdir()) / f"spectro-python-{TARGET_PYTHON_VERSION}-amd64.exe"
...
def install_vc_redist():
    installer = download_file(
        VC_REDIST_URL,
        Path(tempfile.gettempdir()) / "spectro-vc_redist.x64.exe",
...
def create_run_file(project_dir, python_command):
    ...
    run_bat = project_dir / "Run Spectro.bat"
    run_bat.write_text(
        "@echo off\n"
        "cd /d %~dp0\n"
...
def main():
    ...
    if not setup_py.exists() or not main_py.exists():
        print("setup.exe must be placed in the Spectro project folder next to setup.py and main.py.")
        input("Press Enter to close...")
        return 1
    ...
    env = os.environ.copy()
    env["SPECTRO_SETUP_AUTO"] = "1"
    run(python_command + ["setup.py", "--spectro-install"], cwd=project_dir, env=env)
    create_run_file(project_dir, python_command)

    print("")
    print("Spectro setup completed.")
    print("Start your emulator, open Brawl Stars, then run Run Spectro.bat or python main.py.")
    input("Press Enter to close...")
    return 0
