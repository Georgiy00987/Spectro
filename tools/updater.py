...
def download_file(url: str, destination: Path, label: str) -> Path:
    print(f"Downloading latest Spectro update ({label})...")
    request = urllib.request.Request(url, headers={"User-Agent": "Spectro-Updater"})
...
def find_project_root(extracted_dir: Path) -> Path:
    ...
    if not candidates:
        raise FileNotFoundError("Downloaded update does not look like a Spectro project.")
...
def main() -> int:
    ...
    if not (project_dir / "main.py").exists():
        print("updater.exe must be inside the Spectro project folder next to main.py.")
        wait_for_enter()
        return 1

    if "--smoke-test" in sys.argv:
        print("Smoke test passed. Updater can see the Spectro project folder.")
        return 0
    ...
    temp_dir = Path(tempfile.mkdtemp(prefix="spectro_update_"))
    backup_dir = temp_dir / "preserved_user_files"
    zip_path = temp_dir / "latest_spectro.zip"
