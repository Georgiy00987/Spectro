...
from control.window_controller import (
    WindowController,
...
class EmulatorProfileMappingTest(unittest.TestCase):
    ...
    def test_supported_bluestacks_config_never_uses_adb_server_port(self):
        emulator, port = _normalize_emulator_config("BlueStacks", 5037)

        self.assertEqual(emulator, "BlueStacks")
        self.assertEqual(port, 0)

    def test_bluestacks_port_infers_bluestacks(self):
        emulator, port = _normalize_emulator_config("Unknown", 5565)

        self.assertEqual(emulator, "BlueStacks")
        self.assertEqual(port, 5565)
