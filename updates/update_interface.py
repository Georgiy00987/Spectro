...
from updates.update_settings import as_github_update_config, load_update_settings
...
class UpdatePanel:
    ...
    def _config(self):
        return as_github_update_config()
    ...
    def _build(self):
        ...
        self._stat_box(summary, 2, "Source", settings["repo"] if settings["repo"] else "Not configured")
    ...
    def _run_worker(self, action):
        ...
        if not settings["repo"] or "/" not in settings["repo"]:
            self._status("Updater source is not configured.", "#E74C3C")
            self._append_log("Set repo in cfg/update_config.toml under [github_update].")
            return
