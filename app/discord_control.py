...
from app.runtime_control import PAUSED, RUNNING, read_state, write_state
from core.utils import _config_bool
from integrations.discord_notifier import load_webhook_settings
    ...
    async def _run(self, token: str) -> None:
        ...
        async def _guard(interaction: discord.Interaction) -> bool:
            ...
            if command_allowed(
                ...
                return True
            await _reply(interaction, "You are not allowed to control this Spectro bot.")
            return False

        @tree.command(name="stop", description="Pause Spectro.")
        async def stop_command(interaction: discord.Interaction) -> None:
            if not await _guard(interaction):
                return
            set_runtime_state(self.state_path, paused=True)
            await _reply(interaction, "Spectro paused.")

        @tree.command(name="start", description="Resume Spectro.")
        async def start_command(interaction: discord.Interaction) -> None:
            if not await _guard(interaction):
                return
            set_runtime_state(self.state_path, paused=False)
            await _reply(interaction, "Spectro resumed.")

        @tree.command(name="status", description="Show whether Spectro is running or paused.")
        async def status_command(interaction: discord.Interaction) -> None:
            if not await _guard(interaction):
                return
            state = read_state(self.state_path)
            await _reply(interaction, f"Spectro is {'paused' if state == PAUSED else 'running'}.")
