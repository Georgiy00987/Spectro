...
def _build_providers(preferred_device):
    ...
    if preferred_device in ("gpu", "auto", "cuda", "tensorrt", "trt"):
        if "CUDAExecutionProvider" in available_providers:
            cuda_ready, missing_cuda_dlls = has_cuda_dependency_dlls()
            if cuda_ready:
                providers.append((
                ...
                print(
                    "CUDA was requested, but required CUDA/cuDNN DLLs are missing: "
                    f"{', '.join(missing_cuda_dlls)}. Falling back to CPU. "
                    "Install CUDA/cuDNN runtime or switch to DirectML/CPU."
                )
            elif not _provider_message_printed:
                print(
                    "CUDA provider is installed, but required CUDA/cuDNN DLLs are missing: "
                    f"{', '.join(missing_cuda_dlls)}. Skipping CUDA to avoid ONNX Runtime errors."
                )
